...
# ── 【模块配置区】新模块只改这一行（与 baseline.yaml entry_url 一致）─────────
BASE_URL = os.environ.get("TC_BASE_URL", "https://test-risk.inshopline.com")

# flow 清单自动发现（也支持 TC_FLOWS 显式指定子集，逗号分隔）
FLOW_FILES = (
    [f.strip() for f in os.environ.get("TC_FLOWS", "").split(",") if f.strip()]
    or sorted(f for f in os.listdir(BASE_DIR)
              if f.startswith("flow") and f.endswith(".yaml"))
)
...
def get_cookie_from_ego():
    ...
    script = (
        f"const task = await useOrCreateTaskSpace('{os.environ.get('TC_TASKSPACE', 'tc-regression')}')\n"
